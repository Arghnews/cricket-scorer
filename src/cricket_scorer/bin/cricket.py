...
def main():
    ...
    get_keys = lambda profiles: [key for key in profiles.keys()
            if not key.startswith("_")]

    sender_parser = subparsers.add_parser("sender")
    sender_parser.add_argument("--profile", choices = get_keys(
        params.sender_profiles), required = True)

    receiver_parser = subparsers.add_parser("receiver")
    receiver_parser.add_argument("--profile", choices = get_keys(
        params.receiver_profiles), required = True)
    ...
    print("Running cricket program with mode:", mode, "profile:", profile,
            " args:", parsed_args)

    if mode == "sender":
        print("Sender profile:", params.sender_profiles[profile])
