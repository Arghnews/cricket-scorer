...
def i2c_write(bus, log, addr, mux_addr, val):
    ...
    try:
        ...
        log.error("I2c bus write error. Bus:", bus, "addr:", hex(addr),
                "writing raw data:", hex(val), ", error:", str(e))
...
class ScoreWriterI2cMark1:
    def __init__(self, log):
        ...
        muxes = [0x4, 0x5, 0x6]
        self.addrs_muxes = [(a, m) for a in addrs for m in muxes]

    def __call__(self, score, blank_out_leading_zeroes = True):
        score = utils.sanitise_received_score(self.log, score,
                len(self.addrs_muxes), blank_out_leading_zeroes)

        if score is None:
            return

        for val, (addr, mux) in zip(score, self.addrs_muxes):
            self.log.debug("Writing to addr:", addr, "mux:", mux, "value:",
                    val)
            i2c_write(self.bus, self.log, addr, mux, utils.int_to_display[val])
